...
- 六套主题的可信 HTML 都支持原生 `<progress>`、`.md-tag` 和静态 `.md-date-picker` / `.md-time-picker` 展示类；这些 picker 类不提供日期计算、弹层或提交行为。
- `animal-island` 额外提供 `.md-skeleton*` 骨架屏、`.md-back-top` 和 `.md-time-game`，并为共享 tag/picker 使用 v1.6.0 的奶油面板、圆形日期格和琥珀范围视觉。
